...
const DEFAULT_DATA = {
    title: 'Premium Life\nCommerce Brand',
    description: '마리쥬르는 단순한 인테리어 제품을 판매하는 것이 아니라,\n생활하는 공간의 질 높은 라이프 스타일을 디자인할 수 있도록 제안하는\n브랜드입니다. 감성적이고 트렌디한 상품으로 여러분의 공간이 채워지길 바라며,\n고객님과 소통하고 늘 고민하겠습니다.',
    imageUrl: {
        desktop: BANNER_IMAGE_MEDIA.desktop,
        mobile: BANNER_IMAGE_MEDIA.mobile || BANNER_IMAGE_MEDIA.desktop
    },
    height: 500, // 기본 높이 (PC: 500px, 모바일: 400px)
    overlayColor: '#000000',
    overlayOpacity: 0.5,
...
/**
 * BannerImageMediaSkin 컴포넌트 - FixedBgText API 스펙 적용 (고정 배경 사용)
 *
 * 기능:
 ...
 * - 디바이스별 콘텐츠 지원
 */
const BannerImageMediaSkin: React.FC<ComponentSkinProps> = (props = {}) => {
    const { 
        data = {}, 
        actions = {}, 
        utils = {}, 
    ...
    const rootStyle: React.CSSProperties = {
        ...
        backgroundAttachment: 'fixed', // 고정 배경 적용
        minHeight: `${height}px`,
        width: '100%',
        ...data.rootStyle
    };
    ...
    if (!isLoaded) {
        return (
            <div className="bim-skin-loading" style={{ minHeight: `${height}px`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    if (hasError) {
        return (
            <div className="bim-skin-error" style={{ minHeight: `${height}px`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    return (
        <section 
            id={id}
            className="bim-skin-container"
            ref={componentRef}
            style={rootStyle}
            {...dataAttributes}
        >
            {/* 오버레이 */}
            <div className="bim-skin-overlay" style={overlayStyle} />
            
            {/* 콘텐츠 */}
            <div className="bim-skin-content" style={contentStyle}>
                {htmlContent ? (
                    <div 
                        className="bim-skin-html-content" 
                        style={htmlContentStyle}
                        dangerouslySetInnerHTML={{ __html: htmlContent }} 
                    />
                ) : (
                    <div>
                        <h2 className="bim-skin-title" style={{ color: textColor }}>
                            {t(finalTitle)}
                        </h2>
                        <p className="bim-skin-description" style={{ color: textColor }}>
                            {t(finalDescription)}
                        </p>
                    </div>
                )}
            ...
            {/* 디버그 정보 */}
            {debug && (
                <div className="bim-skin-debug-info" style={{
                    position: 'absolute',
                    top: '10px',
                    left: '10px',
                    ...
                    <p>Fixed Background: Yes</p>
                </div>
            )}
            
            {/* 에디터 힌트 */}
            {mode === 'editor' && editor?.isSelected && (
                <div className="bim-skin-editor-hint" style={{
                    position: 'absolute',
                    top: '10px',
                    right: '10px',
...
(BannerImageMediaSkin as any).SAMPLE_DATA = DEFAULT_DATA;

// 기본 익스포트
export default BannerImageMediaSkin;

// 명명된 익스포트 (하위 호환성)
export { BannerImageMediaSkin, BannerImageMediaSkin as BannerImageMedia };